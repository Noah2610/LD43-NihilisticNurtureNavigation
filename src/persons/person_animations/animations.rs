...
pub mod player {
  ...
  pub fn new_idle_animation(ctx: &mut Context) -> Animation {
    let filepaths: Vec<String> = vec![ ::join_str(IMAGES, "child1_1.png") ];
    let interval_ms = vec![ 250 ];
    Animation::new(ctx, filepaths, interval_ms)
  }
  ...
  pub fn new_walk_animation(ctx: &mut Context) -> Animation {
    ...
    let interval_ms_walk = vec![
      250,
      250,
      250,
      250
    ];
    Animation::new(ctx, filepaths_walk, interval_ms_walk)
  }
